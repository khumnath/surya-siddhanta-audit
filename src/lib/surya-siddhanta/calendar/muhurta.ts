 ...
 * 
 * Implements:
 * 1. Choghadia (8 slots for Day and Night)
 * 2. Special Kaalas (Rahu Kaal, Yamaganda, Gulika)
 * 3. Abhijit Muhurta
 */
...
// Day sequences indexed by weekday (0=Sun, 1=Mon, ..., 6=Sat)
const DAY_SEQUENCES = [
  [0, 1, 2, 3, 4, 5, 6, 0], // Sun
  [3, 4, 5, 6, 0, 1, 2, 3], // Mon
  [6, 0, 1, 2, 3, 4, 5, 6], // Tue
...
export function calculateDailyMuhurtas(
  ...
  // 3. Abhijit Muhurta (approx 1/15th of day centered on midday)
  const midDay = (sunriseHours + sunsetHours) / 2.0;
  const abhijitDuration = dayDuration / 15.0;
  results.push({
